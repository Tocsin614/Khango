local function MyRoutine()
    ...
    -- Spells (all ranks)
    ------------------------------------------------------------
    HL.Spell.Druid = HL.Spell.Druid or {}
    local S = HL.Spell.Druid

    -- Shapeshift
    S.DireBearForm = S.DireBearForm or Spell(9634)

    -- Bear Form Abilities
    S.Maul = S.Maul or MultiSpell(
        6807,   -- R1
        6808,   -- R2
        6809,   -- R3
        8972,   -- R4
    ...
    S.MangleBear = S.MangleBear or MultiSpell(
        33878,  -- Mangle (Bear) R1 - Level 50
        33986,  -- Mangle (Bear) R2 - Level 58  
        33987   -- Mangle (Bear) R3 - Level 66
    )
    S.Mangle = S.MangleBear  -- Alias for easier reference

    S.FaerieFireFeral = S.FaerieFireFeral or MultiSpell(
        16857,  -- R1
        17390,  -- R2
        17391,  -- R3
        17392,  -- R4
    ...
    S.DemoralizingRoar = S.DemoralizingRoar or MultiSpell(
        99,     -- R1
        1735,   -- R2
        9490,   -- R3
        9747,   -- R4
    ...
    S.Lacerate = S.Lacerate or Spell(33745)  -- TBC skill
    S.LacerateDebuff = S.LacerateDebuff or Spell(33745)  -- Same ID for debuff tracking

    S.Enrage = S.Enrage or Spell(5229)

    -- Buffs
    S.MarkOfTheWild = S.MarkOfTheWild or MultiSpell(
        1126,   -- R1
        5232,   -- R2
        6756,   -- R3
        5234,   -- R4
    ...
    S.GiftOfTheWild = S.GiftOfTheWild or MultiSpell(
        21849,  -- R1
        21850,  -- R2
        26991   -- R3
    )

    S.Thorns = S.Thorns or MultiSpell(
        467,    -- R1
        782,    -- R2
        1075,   -- R3
        8914,   -- R4
    ...
    S.OmenOfClarity = S.OmenOfClarity or Spell(16864)
    ...
    local MARK_OF_THE_WILD_NAME = GetSpellInfo(1126)
    local GIFT_OF_THE_WILD_NAME = GetSpellInfo(21849)
    local THORNS_NAME = GetSpellInfo(467)
    local OMEN_OF_CLARITY_NAME = GetSpellInfo(16864)
    ...
    local function PlayerHasMarkOfWild()
        return HasBuffByName("player", MARK_OF_THE_WILD_NAME) or HasBuffByName("player", GIFT_OF_THE_WILD_NAME)
    end

    local function PlayerHasThorns()
        return HasBuffByName("player", THORNS_NAME)
    end

    local function PlayerHasOmenOfClarity()
        return HasBuffByName("player", OMEN_OF_CLARITY_NAME)
    end
    ...
    ------------------------------------------------------------
    local function Init()
        MainAddon:Print('Feral Bear Tank - TBC Classic 1.0 loaded (SpecID 11).')
    end
    ...
    local function MainRotation()
        ...
        if autoMark and not PlayerHasMarkOfWild() then
            if useGift then
                if S.GiftOfTheWild:IsReady(Player) then
                    if Cast(S.GiftOfTheWild) then
                        return "Gift of the Wild"
                    end
                end
            else
                if S.MarkOfTheWild:IsReady(Player) then
        ...
        -- Thorns
        if autoThorns and not PlayerHasThorns() then
            if S.Thorns:IsReady(Player) then
        ...
        -- Omen of Clarity
        if autoOmen and not PlayerHasOmenOfClarity() then
            if S.OmenOfClarity:IsReady(Player) then
