local function MyRoutine()
    ...
    -- Spells (all ranks)
    ------------------------------------------------------------
    HL.Spell.Warlock = HL.Spell.Warlock or {}
    local S = HL.Spell.Warlock

    -- Armor
    S.FelArmor = S.FelArmor or MultiSpell(
        28176,  -- R1
        28189   -- R2
    )

    S.DemonArmor = S.DemonArmor or MultiSpell(
        706,    -- R1
        1086,   -- R2
        11733,  -- R3
        11734,  -- R4
    ...
    -- Pets
    S.SummonImp = S.SummonImp or Spell(688)
    S.SummonSuccubus = S.SummonSuccubus or Spell(712)
    S.SummonFelguard = S.SummonFelguard or Spell(30146)  -- Demonology 41-point talent
    S.DemonicSacrifice = S.DemonicSacrifice or Spell(18788)

    -- Curses
    S.CurseOfDoom = S.CurseOfDoom or MultiSpell(
        603,    -- R1
        30910   -- R2
    )

    S.CurseOfAgony = S.CurseOfAgony or MultiSpell(
        980,    -- R1
        1014,   -- R2
        6217,   -- R3
        11711,  -- R4
    ...
    S.CurseOfTheElements = S.CurseOfTheElements or MultiSpell(
        1490,   -- R1
        11721,  -- R2
        11722,  -- R3
        27228   -- R4
    ...
    S.CurseOfRecklessness = S.CurseOfRecklessness or MultiSpell(
        704,    -- R1
        7658,   -- R2
        7659,   -- R3
        11717,  -- R4
    ...
    S.CurseOfWeakness = S.CurseOfWeakness or MultiSpell(
        702,    -- R1
        1108,   -- R2
        6205,   -- R3
        7646,   -- R4
    ...
    -- DoTs
    S.Immolate = S.Immolate or MultiSpell(
        348,    -- R1
        707,    -- R2
        1094,   -- R3
        2941,   -- R4
    ...
    S.Corruption = S.Corruption or MultiSpell(
        172,    -- R1
        6222,   -- R2
        6223,   -- R3
        7648,   -- R4
    ...
    S.DrainLife = S.DrainLife or MultiSpell(
        689,    -- R1
        699,    -- R2
        709,    -- R3
        7651,   -- R4
    ...
    S.DrainSoul = S.DrainSoul or MultiSpell(
        1120,   -- R1
        8288,   -- R2
        8289,   -- R3
        11675,  -- R4
    ...
    -- Fear Spells
    S.Fear = S.Fear or MultiSpell(
        5782,   -- R1
        6213,   -- R2
        6215    -- R3
    )

    S.HowlOfTerror = S.HowlOfTerror or MultiSpell(
        5484,   -- R1
        17928   -- R2
    )

    -- Main Nukes
    S.ShadowBolt = S.ShadowBolt or MultiSpell(
        686,    -- R1
        695,    -- R2
        705,    -- R3
        1088,   -- R4
    ...
    S.Incinerate = S.Incinerate or MultiSpell(
        29722,  -- R1
        32231   -- R2
    )

    -- Instant Casts
    S.Shadowburn = S.Shadowburn or MultiSpell(
        17877,  -- R1
        18867,  -- R2
        18868,  -- R3
        18869,  -- R4
    ...
    S.DeathCoil = S.DeathCoil or MultiSpell(
        6789,   -- R1
        17925,  -- R2
        17926,  -- R3
        27223   -- R4
    ...
    -- AoE
    S.SeedOfCorruption = S.SeedOfCorruption or Spell(27243)

    S.Hellfire = S.Hellfire or MultiSpell(
        1949,   -- R1
        11683,  -- R2
        11684,  -- R3
        27213   -- R4
    ...
    S.RainOfFire = S.RainOfFire or MultiSpell(
        5740,   -- R1
        6219,   -- R2
        11677,  -- R3
        11678,  -- R4
    ...
    -- Mana Management
    S.LifeTap = S.LifeTap or MultiSpell(
        1454,   -- R1
        1455,   -- R2
        1456,   -- R3
        11687,  -- R4
    ...
    S.DarkPact = S.DarkPact or MultiSpell(
        18220,  -- R1
        18937,  -- R2
        18938,  -- R3
        27265   -- R4
    ...
    -- Utility
    S.Conflagrate = S.Conflagrate or MultiSpell(
        17962,  -- R1
        18930,  -- R2
        18931,  -- R3
        18932,  -- R4
    ...
    S.Soulshatter = S.Soulshatter or Spell(29858)  -- TBC threat drop

    S.Shoot = S.Shoot or Spell(5019)  -- Wand attack

    S.SoulLink = S.SoulLink or Spell(19028)  -- Demonology 31-point talent
    ...
    ------------------------------------------------------------
    local function Init()
        MainAddon:Print('Demonology Felguard Warlock - TBC Leveling 50-70 loaded (SpecID 9).')
    end
    ...
    local function EnemyRotation()
        ...
        --------------------------------------------------------
        -- PRIORITY #4: Fear if in danger (low health)
        --------------------------------------------------------
        ...
        --------------------------------------------------------
        -- PRIORITY #5: Drain Life (if need health)
        --------------------------------------------------------
        ...
        --------------------------------------------------------
        -- PRIORITY #6: Drain Soul (execute, shards/mana)
        --------------------------------------------------------
        ...
        --------------------------------------------------------
        -- PRIORITY #7: Wand (filler when mana is low AND DoTs are up)
    ...
    local function MainRotation()
        ...
        -- Summon Felguard if missing (Demonology leveling pet)
