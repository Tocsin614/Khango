local function MyRoutine()
    ...
    -- Spells (all ranks)
    ------------------------------------------------------------
    HL.Spell.Warrior = HL.Spell.Warrior or {}
    local S = HL.Spell.Warrior

    -- Stances
    S.BerserkerStance = S.BerserkerStance or Spell(2458)

    -- Shouts
    S.BattleShout = MultiSpell(
        6673,   -- R1
        5242,   -- R2
        6192,   -- R3
        11549,  -- R4
    ...
    S.CommandingShout = MultiSpell(
        469,    -- R1
        47439,  -- R2
        47440   -- R3
    )

    S.DemoralizingShout = MultiSpell(
        1160,   -- R1
        6190,   -- R2
        11554,  -- R3
        11555,  -- R4
    ...
    S.Whirlwind = S.Whirlwind or Spell(1680)

    S.HeroicStrike = MultiSpell(
        78,     -- R1
        284,    -- R2
        285,    -- R3
        1608,   -- R4
    ...
    -- Cooldowns
    S.Bloodrage = S.Bloodrage or Spell(2687)
    S.DeathWish = S.DeathWish or Spell(12292)
    S.Recklessness = S.Recklessness or Spell(1719)
    S.Rampage = MultiSpell(
        29801,  -- R1
        30030,  -- R2
        30033   -- R3
    )
    ...
    -- AoE
    S.SweepingStrikes = S.SweepingStrikes or Spell(12328)
    S.Cleave = MultiSpell(
        845,    -- R1
        7369,   -- R2
        11608,  -- R3
    ...
    -- Buff/Debuff helpers
    ------------------------------------------------------------
    local BATTLE_SHOUT_NAME = GetSpellInfo(6673)      -- R1
    local COMMANDING_SHOUT_NAME = GetSpellInfo(469)   -- R1
    local DEMORALIZING_SHOUT_NAME = GetSpellInfo(1160) -- R1
    local RAMPAGE_NAME = GetSpellInfo(29801)          -- R1
    local BLOODLUST_NAME = GetSpellInfo(2825)         -- Bloodlust
    local HEROISM_NAME = GetSpellInfo(32182)          -- Heroism
    ...
    local function PlayerHasShout()
        return HasBuffByName("player", BATTLE_SHOUT_NAME)
            or HasBuffByName("player", COMMANDING_SHOUT_NAME)
    end

    local function TargetHasDemoralizingShout()
        return HasDebuffByName("target", DEMORALIZING_SHOUT_NAME)
    end

    local function PlayerHasRampage()
        return HasBuffByName("player", RAMPAGE_NAME)
    end

    local function PlayerHasBloodlust()
    ...
    ------------------------------------------------------------
    local function Init()
        MainAddon:Print('Fury Warrior - TBC Classic 1.0 loaded (SpecID 1).')
    end
    ...
    local function EnemyRotation()
        ...
        --------------------------------------------------------
        if autoDemo and not TargetHasDemoralizingShout() then
        ...
        local hasBloodlust = PlayerHasBloodlust()
        ...
        --------------------------------------------------------
        if useRampage and not PlayerHasRampage() and S.Rampage:IsReady() then
    ...
    local function MainRotation()
        ...
        --------------------------------------------------------
        if autoBShout and not PlayerHasShout() then
            if S.BattleShout:IsReady() then
