local function MyRoutine()
    local Author = 'Retribution Paladin - TBC Classic 1.0'
    ...
    -- Seals
    S.SealOfTheCrusader = S.SealOfTheCrusader or MultiSpell(
        21082,  -- R1
        20162,  -- R2
        20305,  -- R3
        20306,  -- R4
    ...
    S.SealOfCommand = S.SealOfCommand or MultiSpell(
        20375,  -- R1
        20915,  -- R2
        20918,  -- R3
        20919,  -- R4
    ...
    S.SealOfBlood = S.SealOfBlood or Spell(31892)  -- R1 (Horde only)

    S.SealOfTheMartyr = S.SealOfTheMartyr or Spell(348700)  -- R1 (Alliance version of Seal of Blood)
    ...
    -- Blessings
    S.BlessingOfMight = S.BlessingOfMight or MultiSpell(
        19740,  -- R1
        19834,  -- R2
        19835,  -- R3
        19836,  -- R4
    ...
    S.BlessingOfSalvation = S.BlessingOfSalvation or Spell(1038)

    -- Greater Blessings
    S.GreaterBlessingOfMight = S.GreaterBlessingOfMight or MultiSpell(
        25782,  -- R1
        25916   -- R2
    )
    ...
    S.GreaterBlessingOfSalvation = S.GreaterBlessingOfSalvation or Spell(25895)
    ...
    S.SanctityAura = S.SanctityAura or Spell(20218)  -- TBC aura
    ...
    -- Attacks
    S.CrusaderStrike = S.CrusaderStrike or Spell(35395)  -- TBC skill
    ...
    S.HolyWrath = S.HolyWrath or MultiSpell(
        2812,   -- R1
        10318,  -- R2
        27139   -- R3
    )
    ...
    -- Utility
    S.AvengingWrath = S.AvengingWrath or Spell(31884)  -- TBC cooldown
    ...
    local Paladin_Config = {
        ...
        config   = {
            ...
            { type='checkbox', text='Enable Seal Twisting (Level 70 - Advanced)', key='enabletwist', icon=S.SealOfCommand:ID(), default=false },
    ...
    ------------------------------------------------------------
    local function Init()
        MainAddon:Print('Retribution Paladin - TBC Classic 1.0 loaded.')
    end
    ...
    local function EnemyRotation()
        ...
        if playerLevel >= 70 and enableTwist then
            ...
            if not hasBloodSeal and not hasCommandSeal and nextSwing > 1.9 then
        ...
        if useCStrike and S.CrusaderStrike:IsReady() then
            -- Only use here if NOT twisting at 70 (twisting uses absolute priority)
            if not (playerLevel >= 70 and enableTwist) then
                if Cast(S.CrusaderStrike) then
                    return "Crusader Strike"
                end
            end
            ...
            if hasCrusaderDebuff and autoBlood then
                ...
                if enableTwist then
                    ...
                    if hasBloodSeal and S.Judgement:IsReady() 
                       and (nextSwing > 0.5 or not Target:IsInRange(10)) then
                        if Cast(S.Judgement) then
