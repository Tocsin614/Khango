local function MyRoutine()
	...
	-- Spell table
	------------------------------------------------------------
	HL.Spell.Shaman = HL.Spell.Shaman or {}
	local S = HL.Spell.Shaman

	------------------------------------------------------------
	-- Spells (all ranks for 1–70 where applicable)
	------------------------------------------------------------

	-- Auto Attack
	S.Attack = S.Attack or Spell(6603) -- Auto Attack

	-- Weapon imbues
	S.RockbiterWeapon = S.RockbiterWeapon or MultiSpell(
	...
	S.WindfuryWeapon = S.WindfuryWeapon or MultiSpell(
	...
	-- Shields
	S.WaterShield = MultiSpell(
		24398,	-- Water Shield (Rank 1)
		33736	-- Water Shield (Rank 2 - TBC)
	)

	S.LightningShield = S.LightningShield or MultiSpell(
	...
	-- Pull spell
	S.LightningBolt = S.LightningBolt or MultiSpell(
	...
	-- Shocks / Damage
	------------------------------------------------------------
	S.EarthShock = S.EarthShock or MultiSpell(
	...
	S.FlameShock = S.FlameShock or MultiSpell(
	...
	S.Stormstrike        = S.Stormstrike        or Spell(17364) -- Stormstrike
	S.ShamanisticRage    = S.ShamanisticRage    or Spell(30823) -- Shamanistic Rage
	S.FireElementalTotem = S.FireElementalTotem or Spell(2894)  -- Fire Elemental Totem
	...
	S.StrengthOfEarthTotem = S.StrengthOfEarthTotem or MultiSpell(
	...
	S.StoneclawTotem = S.StoneclawTotem or MultiSpell(
	...
	S.GraceOfAirTotem = S.GraceOfAirTotem or MultiSpell(
	...
	-- Air: Windfury Totem
	S.WindfuryTotem = S.WindfuryTotem or MultiSpell(
	...
	S.ManaSpringTotem = S.ManaSpringTotem or MultiSpell(
	...
	-- Fire: Searing Totem
	S.SearingTotem = S.SearingTotem or MultiSpell(
	...
	-- Fire: Magma Totem
	S.MagmaTotem = S.MagmaTotem or MultiSpell(
	...
	S.FireNovaTotem = S.FireNovaTotem or MultiSpell(
	...
	------------------------------------------------------------
	-- Helper spell names for buff/debuff checks
	------------------------------------------------------------
	local WATER_SHIELD_NAME     = GetSpellInfo(24398)
	local LIGHTNING_SHIELD_NAME = GetSpellInfo(324)
	...
	------------------------------------------------------------
	-- Enemy count helper (Destro-style): Player:GetEnemiesInRange(range) then #table
	------------------------------------------------------------
	...
	local function Init()
		TotemTwistTimer = 0
		TotemTwistState = 0
		MainAddon:Print('Shaman - Enhancement (TBC) 1.9 loaded...')
	end
	...
	local function LevelingRotation()
		...
		if not MainAddon.TargetIsValid() then
			if useLS
				...
				and not HasBuffByName("player", LIGHTNING_SHIELD_NAME)
			then
	...
	------------------------------------------------------------
	-- NORMAL / RAID ROTATION (APL style)
	------------------------------------------------------------
	local function EnemyRotation()
		...
		if not MainAddon.TargetIsValid() then
			...
			-- APL 2: Water Shield - self check uses IsReady(Player)
			if useWaterShield
				and S.WaterShield:IsReady(Player)
				and not Player:BuffUp(S.WaterShield)   --proper way o check buff since Rubim fixed multispell
			then
		...
		if useWaterShield
			and S.WaterShield:IsReady(Player)
			and not Player:BuffUp(S.WaterShield)
		then
			if Cast(S.WaterShield) then -- APL 2
...
------------------------------------------------------------
-- Loader loop (same pattern as Hunter)
------------------------------------------------------------
